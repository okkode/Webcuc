...
namespace GdeWebAPI.Services
{
    ...
    public class HostedService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    var _noteService = scope.ServiceProvider.GetRequiredService<INoteService>();

                    // MONTHLY SUMMARY GENERATION - Run on the first day of each month
                    await GenerateMonthlySummaries(_noteService, _logService, scope);
        ...
        /// </summary>
        #region GENERATE MONTHLY SUMMARIES
        private async Task GenerateMonthlySummaries(INoteService noteService, ILogService logService, IServiceScope scope)
        {
            try
            {
                var now = DateTime.UtcNow;
                
                // Only run on the first day of the month, and only once per day
                if (now.Day == 1 && now.Hour == 2) // Run at 2 AM on the 1st of each month
                {
                    ...
                    foreach (var userId in users)
                    {
                        try
                        {
                            ...
                            var existingSummary = await noteService.GetMonthlySummary(userId, year, month);
                            ...
                            // Get all notes from the previous month
                            var allNotes = await noteService.GetUserNotes(userId);
                            if (!allNotes.Result.Success || allNotes.NoteList.Count == 0)
                            {
                                continue;
                            }

                            // Filter notes from the previous month
                            ...
                            var aiResponse = await _ai.GenerateMonthlySummaryAsync(notesContent, year, month);
                            
                            // Parse the response
                            var (summary, whatLearned, whatPresented) = _ai.ParseMonthlySummary(aiResponse);
                            ...
                            var result = await noteService.AddOrUpdateMonthlySummary(summaryModel);
                            if (!result.Success)
                            {
                                await logService.WriteMessageLogToFile(
                                    $"Monthly summary generation failed for UserId: {userId}, Year: {year}, Month: {month}, Error: {result.ErrorMessage}",
                                    "GenerateMonthlySummaries");
                            }
                        }
                        catch (Exception ex)
                        {
                            await logService.WriteLogToFile(ex, $"GenerateMonthlySummaries for UserId: {userId}");
                        }
                    }
                }
            }
            ...
            {
                await logService.WriteLogToFile(ex, "GenerateMonthlySummaries");
            }
        }
        #endregion
    }
